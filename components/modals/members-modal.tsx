...
export const MembersModal = () => {
  ...
  return (
    <Dialog open={isModalopen} onOpenChange={onClose}>
      <DialogContent className=" bg-transperent shadow-zinc-900 border-0 text-white overflow-hidden ">
          ...
          {server?.members?.map((member) => (
            <div key={member.id} className="flex items-center gap-x-2 mb-6">
              ...
              <div className="flex flex-col gap-y-1">
                <div className="text-xs font-bold capitalize flex items-center gap-x-2">
                  {member.profile.name}
              ...
              {loadingId === member.id && (
                <Loader2 className="animate-spin text-zinc-200 ml-auto w-4 h-4" />
              )}
            </div>
          ))}
        </ScrollArea>
